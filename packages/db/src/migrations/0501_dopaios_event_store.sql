-- 0501: event store Dopaios theo blueprint message-db (KC-01).
-- Nguồn: dopaios/message-db/ (pin 25da82b044f94416202ac3daa1866791b385badc, v1.3.0)
-- — xem dopaios/message-db/PIN.md. DDL nhúng verbatim theo thứ tự install.sh.
-- Deviation so với install.sh (Dopai sở hữu, lý do trong hồ sơ KC-01):
--   1. Bỏ roles/ và privileges/: ứng dụng kết nối bằng user dịch vụ của
--      deployment, không tạo role message_store riêng trong migration.
--   2. Thêm ALTER FUNCTION ... SET search_path sau khi tạo: thân function của
--      blueprint tham chiếu tên không qualify nên phải tự chứa search_path,
--      không phụ thuộc session ứng dụng.
--   3. SET search_path tạm trong phiên migration để view resolve đúng bảng.
SET search_path TO message_store, public;
--> statement-breakpoint
-- ===== schema/message-store.sql (verbatim) =====
CREATE SCHEMA IF NOT EXISTS message_store;
--> statement-breakpoint
-- ===== extensions/pgcrypto.sql (verbatim) =====
CREATE EXTENSION IF NOT EXISTS pgcrypto;
--> statement-breakpoint
-- ===== tables/messages.sql (verbatim) =====
...
END$$;
--> statement-breakpoint
-- ===== functions/message-store-version.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/hash-64.sql (verbatim) =====
...
IMMUTABLE;
--> statement-breakpoint
-- ===== functions/acquire-lock.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/category.sql (verbatim) =====
...
IMMUTABLE;
--> statement-breakpoint
-- ===== functions/is-category.sql (verbatim) =====
...
IMMUTABLE;
--> statement-breakpoint
-- ===== functions/cardinal-id.sql (verbatim) =====
...
IMMUTABLE;
--> statement-breakpoint
-- ===== functions/stream-version.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/write-message.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/get-stream-messages.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/get-category-messages.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== functions/get-last-stream-message.sql (verbatim) =====
...
VOLATILE;
--> statement-breakpoint
-- ===== indexes/messages-id.sql (verbatim) =====
...
--> statement-breakpoint
-- ===== indexes/messages-stream.sql (verbatim) =====
...
--> statement-breakpoint
-- ===== indexes/messages-category.sql (verbatim) =====
...
--> statement-breakpoint
-- ===== views/stream-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.stream_summary AS
  ...
  ORDER BY
    stream_name;
--> statement-breakpoint
-- ===== views/type-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.type_summary AS
  ...
  ORDER BY
    type;
--> statement-breakpoint
-- ===== views/stream-type-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.stream_type_summary AS
  ...
  ORDER BY
    ...
    type;
--> statement-breakpoint
-- ===== views/type-stream-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.type_stream_summary AS
  ...
  ORDER BY
    ...
    stream_name;
--> statement-breakpoint
-- ===== views/category-type-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.category_type_summary AS
  ...
  ORDER BY
    ...
    type;
--> statement-breakpoint
-- ===== views/type-category-summary.sql (verbatim) =====
CREATE OR REPLACE VIEW message_store.type_category_summary AS
  ...
  ORDER BY
    ...
    category;
--> statement-breakpoint
-- ===== Dopai-owned: function tự chứa search_path =====
ALTER FUNCTION message_store.message_store_version() SET search_path TO message_store, public;
ALTER FUNCTION message_store.hash_64(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.acquire_lock(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.category(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.is_category(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.id(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.cardinal_id(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.stream_version(varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.write_message(varchar, varchar, varchar, jsonb, jsonb, bigint) SET search_path TO message_store, public;
ALTER FUNCTION message_store.get_stream_messages(varchar, bigint, bigint, varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.get_category_messages(varchar, bigint, bigint, varchar, bigint, bigint, varchar) SET search_path TO message_store, public;
ALTER FUNCTION message_store.get_last_stream_message(varchar, varchar) SET search_path TO message_store, public;
--> statement-breakpoint
