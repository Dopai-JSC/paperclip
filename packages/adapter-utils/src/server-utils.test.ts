...
describe("runChildProcess", () => {
  ...
  it("rejects when the child closes before onSpawn persistence fails", async () => {
    const run = runChildProcess(randomUUID(), process.execPath, ["-e", "process.exit(0);"], {
      cwd: process.cwd(),
      env: {},
      timeoutSec: 5,
      graceSec: 1,
      ...
      onSpawn: async () => {
        ...
        throw new Error("database unavailable after child exit");
      },
    });

    await expect(run).rejects.toThrow("database unavailable after child exit");
  });
