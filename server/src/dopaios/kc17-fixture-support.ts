...
// giả lập từ ĐÚNG fixture FX-05 của danh mục V-09 — đọc file thật, đối
// chiếu hash pin trong fx-05-cutover-sim.json, không chép nội dung.
...
export const EFFECTIVE_AT = PLAN_DOC.planned_effective_at; // QD-6: đúng mốc Plan đã pin
...
// Đăng ký một "bản plan" trong sổ cái (id tùy biến; nội dung mặc định =
// đúng byte fixture, có thể override cho ca âm plan-thiếu-trường) rồi đưa
// qua chuỗi KC-03: in-review → Gói quyết định (pointId) → Approval Record.
export async function stagePlanWithApproval(
...
): Promise<{ approvalRecordId: string; sha256: string }> {
  const content = input.contentUtf8 ?? PLAN_CONTENT_UTF8;
  ...
  await assembleDecisionPackage(db, cmd(`pkg-${input.artifactId}`), {
    ...
    revision: 1,
    target: { artifactId: input.artifactId, revision: 1, sha256 },
  ...
  return { approvalRecordId: input.approvalRecordId, sha256 };
}
